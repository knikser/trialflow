locals {
  ...
  # Non-secret environment variables for the .NET monolith
  monolith_env_vars = [
    { name = "APP_ENV", value = local.env },
    { name = "LOG_LEVEL", value = "debug" },
    { name = "AUTH0_DOMAIN", value = "trialflow-staging.auth0.com" },
  ...
  # Non-secret environment variables for the Go application service
  ...
  # Secret ARN references — values injected by ECS at runtime from Secrets Manager
  service_secrets = [
